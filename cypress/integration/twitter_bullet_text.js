...
describe('Test twitter image caption page.', function () {
    it('Select page and assert url', function () {
        ...
        cy.contains('Bullet Text').click();
        cy.url().should('eq', Cypress.env('Test_env') + '/' + this.data[2].url + '/')
    })
    it('Verify Tool Name', function () {
        cy.get('.formHeader > .title').should('have.text', this.data[2].toolName)
    })

    it('Verify Sub-title', function () {
        cy.get('.formHeader > .subtitle').should('have.text', this.data[2].introtext)
    })

    it('Verify I1-Label', function () {
        cy.get('.inputTitle').should('have.text', this.data[2].i1label)
    })

    it('Verify I1-placeholder', function () {
        cy.get('.MuiInputBase-inputMultiline').should('have.text', this.data[2].i1sampletext)
    })
    ...
    it('Verify Sample Output', function () {
        cy.get('.description').should('have.text', this.data[2].sampleoutput)
    })
    ...
    it('Visit the page with user id', function () {
        cy.visit('https://testing.inkforall.com/tools/twitter-bullet-list/?code=4ds34s-231sed2-123sde-32s2332')
