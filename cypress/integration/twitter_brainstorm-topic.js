...
describe('Test twitter image caption page.', function () {
  ...
  it('Verify Sub-title', function () {
    cy.get('.formHeader > .subtitle').should('have.text', this.data[0].introtext)
  })

  it('Verify I1-Label', function () {
    cy.get('.inputTitle').should('have.text', this.data[0].i1label)
  })

  it('Verify I1-placeholder', function () {
    cy.get('.MuiInputBase-inputMultiline').should('have.text', this.data[0].i1sampletext)
  })
  ...
  it('Verify Sample Output', function () {
    cy.get('.description').should('have.text', this.data[0].sampleoutput)
  })
  ...
  it('Visit the page with user id', function () {
    cy.visit('https://testing.inkforall.com/tools/twitter-topic-brainstorm/?code=4ds34s-231sed2-123sde-32s2332')
